...
public extension MeshStore {
    /// Load the observations with `rx_time` in `[since, until]`, oldest first, for
    /// replay. `gateway_id` is stored as the hex node id string (e.g. "!a1b2c3d4")
    /// or a decimal node num; both are parsed to the numeric node id used as the
    /// position-map key. Rows with no gateway keep a nil gateway (drawn as a direct
    /// source→… edge by the trace builder).
    func timelineObservations(since: Instant, until: Instant) async throws -> [TimelineObservation] {
        ...
        let rows: [ObservationRecord] = try await writer.read { db in
            try ObservationRecord
                .filter(Column("rx_time") >= sinceNanos && Column("rx_time") <= untilNanos)
                .order(Column("rx_time"))
                .fetchAll(db)
        }
