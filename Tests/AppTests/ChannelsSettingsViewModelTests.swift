...
struct ChannelsSettingsViewModelTests {
    ...
    func `setting a key flips hasKey and never re-exposes the plaintext`() async throws {
        ...
        // The store holds the true PSK; the UI's ChannelEntry never does. Read it
        // under the channel's *current* hash: a custom PSK re-keys a name-derived
        // channel under its correct on-wire hash (Finding 15).
        let entry = try #require(vm.mqttChannels.first)
        let stored = try #require(await store.storedKey(forChannelHash: entry.hash))
    ...
    func `rotating a key replaces the stored secret`() async throws {
        ...
        // Each setKey re-derives the on-wire hash from the entered PSK and re-files
        // the channel there (Finding 15), so read the stored secret under the
        // channel's current hash after every rotation.
        await vm.setKey(forChannelHash: try #require(vm.localChannels.first).hash, pskText: customKeyB64)
        let firstHash = try #require(vm.localChannels.first).hash
        let first = try #require(await store.storedKey(forChannelHash: firstHash))

        let other = Data(Array(repeating: UInt8(0x77), count: 16)).base64EncodedString()
        await vm.setKey(forChannelHash: firstHash, pskText: other)
        let secondHash = try #require(vm.localChannels.first).hash
        let second = try #require(await store.storedKey(forChannelHash: secondHash))
        ...
        // The stale hash row no longer holds the secret once the channel migrated.
        if firstHash != secondHash {
            #expect(await store.storedKey(forChannelHash: firstHash) == nil)
        }
    }
...
struct ChannelCustomPSKReKeyTests {
    ...
    @Test
    func `a packet whose hash is derived from the custom PSK resolves to that channel`() async throws {
        let (vm, store) = makeVM()
        await vm.addChannel(name: "OpsNet", kind: .mqtt)
        let defaultHash = ChannelKeyMath.channelHash(name: "OpsNet", psk: ChannelKeyMath.defaultPSK)
        let customPSK = [UInt8](Data(base64Encoded: customKeyB64) ?? Data())
    ...
    @Test
    func `the default-PSK shortcut leaves the name-derived hash unchanged`() async throws {
        let (vm, store) = makeVM()
    ...
    @Test
    func `an explicit observed hash is not re-keyed when a custom PSK is set`() async throws {
        let (vm, store) = makeVM()
