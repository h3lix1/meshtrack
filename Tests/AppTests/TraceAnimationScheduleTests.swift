...
struct TraceAnimationScheduleHorizonTests {
    ...
    @Test("empty traces yield the nothing-pending sentinel")
    func emptyHorizon() {
    ...
    @Test("sequential horizon scales with hop count")
    func sequentialHorizon() {
    ...
    @Test("equaliseFinish horizon is one hopDuration regardless of hop count")
    func equaliseFinishHorizon() {
    ...
    @Test("horizon uses max(maxHopIndex, hops) so a collapsed multi-hop path isn't understated")
    func collapsedMultiHopUsesHops() {
...
struct TraceAnimationScheduleFrameTests {
    ...
    @Test("frames are spaced ~frameInterval apart and end just past the horizon")
    func spacingAndEnd() throws {
    ...
    @Test("a start already past the horizon yields exactly one frame")
    func startPastHorizon() {
    ...
    @Test("the nothing-pending sentinel yields exactly one frame")
    func nothingPendingYieldsOne() {
    ...
    @Test("the paused/static variant yields exactly one frame")
    func pausedYieldsOne() {
    ...
    @Test("a non-nothing-pending static schedule still yields exactly one frame")
    func staticWithHorizonYieldsOne() {
    ...
    @Test("entries(from:) returns the same finite frame stream as frameDates")
    func entriesMatchFrameDates() {
